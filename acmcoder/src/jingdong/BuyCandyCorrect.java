...
public class BuyCandyCorrect {
	...
	static class Candy implements Comparable<Candy> {
		...
		Candy(int t, int p, int num) {
			this.p = p;//the type
			this.t = t;//the weight
			this.num = num;//the number
		}

		@Override
		public int compareTo(Candy b) {
			//first order by p (desc) then order by t(asc) then order by number(asc)
			if (b.p == this.p) {
				int r = this.t - b.t;
				if (r == 0)
	...
	public static void main(String[] args) throws IOException {
		...
		while (line != null && line.trim().length() > 0) {
			...
			while (v > 1) {
				//a1 still have value and a2 has no value or a1 current value bigger than a2 current value
				if (a1count < a1.size()
					...
					//the sum two serial value in a1 is bigger than a2
					best.add(a1.get(a1count));
					maxCapacity += a1.get(a1count).p;
					a1count++;
					v--;
				} else {
					//add the a2 current value and before that you should test if there still have a2 value
					if (a2Count >= a2.size()) {
						break;
					}
